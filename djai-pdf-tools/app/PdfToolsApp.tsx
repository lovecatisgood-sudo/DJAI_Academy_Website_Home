...
export default function PdfToolsApp({ language, initialTool, seoPage, initialOptions, acceptedTypes, acceptOverride, fileTypeLabel }: { language: Language; initialTool?: ToolSlug; seoPage?: PdfSeoPage; initialOptions?: Partial<ProcessingOptions>; acceptedTypes?: string[]; acceptOverride?: string; fileTypeLabel?: string }) {
  ...
  const localizedHref = (targetLanguage: Language) => initialTool
    ? `${BASE_PATH}/${publicSlug}/${targetLanguage === "th" ? "" : `${targetLanguage}/`}`
    : homeHref(targetLanguage);
  const thaiHref = localizedHref("th");
  const englishHref = localizedHref("en");
  const vietnameseHref = localizedHref("vi");
    ...
    <main id="top">
      ...
      <header className="site-header">
        ...
        <nav className={menuOpen ? "open" : ""} aria-label="Main navigation">
          ...
          {!(!en && !vi) && <a className="language-link" href={thaiHref} hrefLang="th">ไทย</a>}
          {!en && <a className="language-link" href={englishHref} hrefLang="en">EN</a>}
          {!vi && <a className="language-link" href={vietnameseHref} hrefLang="vi">VI</a>}
        </nav>
      </header>
