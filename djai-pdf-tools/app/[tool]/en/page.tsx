...
export async function generateMetadata({ params }: { params: Promise<{ tool: string }> }): Promise<Metadata> {
  ...
  if (alias) {
    ...
    return {
      title: `${copy.title} | DJTools by DJAI Academy`, description: copy.description, keywords: alias.keywords.en,
      alternates: { canonical: pdfAliasHref(alias, "en"), languages: { th: pdfAliasHref(alias, "th"), en: pdfAliasHref(alias, "en"), vi: pdfAliasHref(alias, "vi"), "x-default": pdfAliasHref(alias, "th") } },
...
export default async function EnglishToolPage({ params }: { params: Promise<{ tool: string }> }) {
  const { tool } = await params;
  const alias = pdfSeoAliases[tool];
  if (alias) return <PdfToolsApp language="en" initialTool={alias.tool} seoPage={alias.copy.en} initialOptions={alias.initialOptions} acceptedTypes={alias.acceptedTypes} acceptOverride={alias.accept} fileTypeLabel={alias.fileTypeLabel} />;
