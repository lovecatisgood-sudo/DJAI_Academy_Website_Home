...
const tools = [
  ["Chuyển video thành văn bản", "Tạo bản chép lời bằng AI", "Chuyển video hoặc audio thành TXT, SRT, VTT hay JSON. Không cần đăng ký; dữ liệu media không được tải lên máy chủ DJAI.", "/tools/video-to-text/vi/", ["Phụ đề", "AI cục bộ"]],
  ["Kiểm tra SEO", "SEO Screaming Toad", "Thu thập website để kiểm tra canonical, hreflang, sitemap, structured data và lỗi kỹ thuật bằng bằng chứng có thể đối chiếu.", "/tools/seo-screaming-toad/vi/", ["SEO kỹ thuật", "Mã nguồn mở"]],
  ...
  ["Âm thanh và video", "Chuyển đổi và nén media", "Chuyển MP3, WAV, M4A, MP4, MOV và WebM; cắt, ghép, resize hoặc nén video ngay trong trình duyệt.", "/tools/media/vi/", ["FFmpeg", "Xử lý cục bộ"]],
...
const workflows = [
  ["Xóa nền ảnh / BG remover", "/tools/resizeimg/remove-background-image/vi/"], ["Đổi HEIC sang JPG", "/tools/resizeimg/heic-to-jpg/vi/"],
  ["Nén ảnh còn khoảng 100 KB", "/tools/resizeimg/image-to-100kb/vi/"], ["Ghép JPG thành PDF", "/tools/PDFTools/jpg-to-pdf/vi/"],
  ["Tách trang PDF thành JPG", "/tools/PDFTools/pdf-to-jpg/vi/"], ["Chuyển DOCX sang PDF", "/tools/document/docx-to-pdf/vi/"],
...
export default function VietnameseToolsPage() {
  return <>
    ...
    <main className="tools-page">
      ...
      <section className="workflow-section" aria-labelledby="workflow-heading"><div><p className="eyebrow">BẮT ĐẦU TỪ VIỆC CẦN LÀM</p><h2 id="workflow-heading">Lối tắt cho những tác vụ thường gặp</h2><p>Mỗi lối tắt mở đúng trang tiếng Việt của tác vụ, kèm hướng dẫn, giới hạn và liên kết tải kết quả.</p></div><div className="workflow-links">{workflows.map(([title, href]) => <a href={href} key={title}><strong>{title}</strong><span>Mở công cụ</span></a>)}</div></section>
