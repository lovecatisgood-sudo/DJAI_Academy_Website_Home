...
const tools = [
  ["Công cụ thương hiệu", "Tạo Favicon và biểu tượng website", "Tạo favicon.ico, biểu tượng PNG, Apple touch icon, maskable icon, web manifest và mã cài đặt từ một hình ảnh ngay trong trình duyệt.", "/tools/brand/vi/", ["Favicon", "Riêng tư"]],
...
const workflows = [
  ["Tạo favicon từ hình ảnh", "/tools/brand/favicon-generator/vi/"],
  ...
  ["Tách trang PDF", "/tools/PDFTools/split-pdf/vi/"], ["Chuyển DOCX sang PDF", "/tools/document/docx-to-pdf/vi/"],
...
export default function VietnameseToolsPage() {
  return <>
    ...
    <main className="tools-page">
      ...
      <section className="workflow-section" aria-labelledby="workflow-heading"><div><p className="eyebrow">BẮT ĐẦU TỪ VIỆC CẦN LÀM</p><h2 id="workflow-heading">Lối tắt cho những tác vụ thường gặp</h2><p>Các nhóm công cụ chính hiện có giao diện tiếng Việt và không yêu cầu đăng ký cho thao tác miễn phí.</p></div><div className="workflow-links">{workflows.map(([title, href]) => <a href={href} key={title}><strong>{title}</strong><span>Mở công cụ</span></a>)}</div></section>
