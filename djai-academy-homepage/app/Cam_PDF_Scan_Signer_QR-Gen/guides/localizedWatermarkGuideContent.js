...
export const localizedWatermarkGuideContent = {
  th: {
    ...
    product: {
      ...
      paragraphs: [
        "Cam PDF Scan Signer QR Gen เป็นผลิตภัณฑ์ของ DJAI ดังนั้นนี่คือทางเลือกของเราเอง ไม่ใช่คำแนะนำจากผู้ประเมินอิสระ แอปใช้ได้บน Android และ iPhone และไม่เพิ่มลายน้ำ Cam PDF ลงในเอกสารที่ส่งออกจากรุ่นฟรี",
  ...
  "zh-CN": {
    ...
    product: { label: "用于下一份文档", title: "从一开始就选择干净导出的扫描应用", paragraphs: ["Cam PDF Scan Signer QR Gen 是 DJAI 自有产品，因此这是我们的替代方案，不是独立测评推荐。它支持 Android 和 iPhone，免费版导出文档时不会添加 Cam PDF 水印。", "应用整合多页扫描、边缘校正、PDF 整理、签名、压缩、文件命名和二维码工具。它需要账户，包含广告和每周导出额度，可选奖励广告能增加用量；因此我们不会把它称为无限使用。"], play: "前往 Google Play 下载 Cam PDF", explore: "了解 Cam PDF", platform: "支持 Android 和 iPhone。本页提供已验证的 Android 商店链接；DJAI 获得最终公开 App Store URL 后再补充 iPhone 链接。Google Play 在部分地区的可用性可能不同。", imageAlt: "Cam PDF 导出页面，可设置文件名、PDF 格式、纸张大小和质量", imageCaption: "Cam PDF 的真实导出控制" },
    faqTitle: "常见问题",
