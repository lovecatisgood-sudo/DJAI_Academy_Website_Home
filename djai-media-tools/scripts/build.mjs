...
export const tools = {
  "mp3-to-wav": { input: ".mp3,audio/mpeg", output: "wav", th: ["แปลง MP3 เป็น WAV ฟรี ออนไลน์", "แปลง MP3 เป็น WAV ฟรี", "แปลงไฟล์ MP3 เป็น WAV ฟรีใน browser เหมาะกับงานตัดต่อ เสียงคุณภาพสูง และโปรแกรมที่ต้องการ WAV"], en: ["Convert MP3 to WAV Free Online", "Convert MP3 to WAV for free", "Convert MP3 audio to WAV free in your browser without upload for editing, lossless workflows, and software that requires WAV."] },
  "wav-to-mp3": { input: ".wav,audio/wav", output: "mp3", th: ["แปลง WAV เป็น MP3 ฟรี ออนไลน์", "แปลง WAV เป็น MP3 ฟรี", "แปลง WAV เป็น MP3 ฟรี พร้อมเลือก bitrate เพื่อลดขนาดไฟล์สำหรับแชร์ ฟัง และ upload"], en: ["Convert WAV to MP3 Free Online", "Convert WAV to MP3 for free", "Convert WAV audio to MP3 free without upload. Choose a bitrate for smaller files, sharing, and uploads."] },
  "m4a-to-mp3": { input: ".m4a,audio/mp4,audio/x-m4a", output: "mp3", th: ["แปลง M4A เป็น MP3 ฟรี ออนไลน์", "แปลง M4A เป็น MP3 ฟรี", "แปลงไฟล์เสียง M4A เป็น MP3 ฟรีใน browser เพื่อใช้กับเครื่องเล่นและโปรแกรมที่รองรับ MP3"], en: ["Convert M4A to MP3 Free Online", "Convert M4A to MP3 for free", "Convert M4A audio to MP3 free in your browser without upload for players, editors, and software that supports MP3."] },
...
const legacyRelatedMedia = {
  "mp3-to-wav": "wav-to-mp3",
  "wav-to-mp3": "mp3-to-wav",
  "m4a-to-mp3": "mp3-to-wav",
  "mp4-to-mp3": "extract-audio-from-video",
  "extract-audio-from-video": "mp4-to-mp3",
  "mp4-to-webm": "webm-to-mp4",
  "webm-to-mp4": "mp4-to-webm",
  "mov-to-mp4": "compress-video",
  "compress-video": "video-cutter"
};

function mediaSuccessJourney(slug, relatedSlug, lang, hidden = false) {
  const isEn = lang === "en";
  return `<aside class="media-success-journey" data-media-success${hidden ? " hidden" : ""}><p class="eyebrow">${isEn ? "CONTINUE THE WORKFLOW" : "ทำงานต่อ"}</p><div class="media-success-links"><a data-related-media-tool href="${href(relatedSlug, lang)}"><strong>${isEn ? "Open a related media tool" : "เปิดเครื่องมือ media ที่เกี่ยวข้อง"}</strong><span>${isEn ? "Continue with the closest conversion or editing task" : "ทำงานแปลงหรือตัดต่อขั้นถัดไปด้วยเครื่องมือที่ตรงงาน"}</span></a><a data-media-development href="${isEn ? "/development/en/" : "/development/"}"><strong>${isEn ? "Build a production media workflow" : "พัฒนา media workflow สำหรับใช้งานจริง"}</strong><span>${isEn ? "Talk to DJAI when this step needs product or automation integration" : "คุยกับ DJAI เมื่อขั้นตอนนี้ต้องเชื่อมกับ product หรือ automation"}</span></a></div></aside>`;
}
...
function render(slug, config, lang, pagePath, pageCopy) {
  ...
  const isEn = lang === "en";
  const successJourney = pagePath ? "" : mediaSuccessJourney(slug, legacyRelatedMedia[slug] || "video-converter", lang, true);
  const taskLinks = [
...
function chineseChrome(locale, slug) {
  ...
  return {
    header: `<header><a class="brand" href="/${locale.segment}/">DJAI <span>Media Tools</span></a><nav><a href="/tools/${locale.segment}/">${tw ? "全部工具" : "全部工具"}</a><a href="${chineseHref(slug, other)}" hreflang="${other.locale}">${locale.switchLabel}</a><a class="community" href="/development/${locale.segment}/">DJAI Development</a></nav></header>`,
    footer: `<footer><span>DJAI Academy</span><a href="/development/${locale.segment}/">DJAI Development</a></footer>`
  };
}
