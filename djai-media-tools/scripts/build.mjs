...
const vietnameseMediaNames = {
  "mp3-to-wav": "Chuyển MP3 sang WAV", "wav-to-mp3": "Chuyển WAV sang MP3", "m4a-to-mp3": "Chuyển M4A sang MP3", "mp4-to-mp3": "Chuyển MP4 sang MP3", "extract-audio-from-video": "Tách âm thanh từ video", "mp4-to-webm": "Chuyển MP4 sang WebM", "webm-to-mp4": "Chuyển WebM sang MP4", "mov-to-mp4": "Chuyển MOV sang MP4", "compress-video": "Nén video",
  "video-converter": "Chuyển đổi định dạng video", "mkv-to-mp4": "Chuyển MKV sang MP4", "avi-to-mp4": "Chuyển AVI sang MP4", "mp4-to-mov": "Chuyển MP4 sang MOV", "video-cutter": "Cắt video", "video-cropper": "Crop video", "video-resizer": "Resize video", "video-merger": "Ghép video", "compress-video-to-10mb": "Nén video xuống khoảng 10 MB", "compress-video-to-25mb": "Nén video xuống khoảng 25 MB", "compress-video-to-50mb": "Nén video xuống khoảng 50 MB", "compress-video-to-100mb": "Nén video xuống khoảng 100 MB", "video-to-gif": "Chuyển video sang GIF", "gif-to-mp4": "Chuyển GIF sang MP4", "remove-audio-from-video": "Xóa âm thanh khỏi video", "add-audio-to-video": "Thêm âm thanh vào video", "video-speed-changer": "Thay đổi tốc độ video", "extract-frames-from-video": "Trích xuất khung hình từ video", "rotate-video": "Xoay video"
};

for (const [slug, config] of Object.entries(tools)) {
  const name = vietnameseMediaNames[slug];
  config.vi = [`${name} miễn phí online`, `${name} miễn phí`, `${name} ngay trong trình duyệt. File không bị upload, không cần đăng ký và kết quả không có watermark.`];
}

function socialHead({ title, description, canonical, lang }) {
  const locale = lang === "vi" ? "vi_VN" : lang === "en" ? "en_US" : "th_TH";
  const alternateLocale = lang === "th" ? "en_US" : "th_TH";
  const imageAlt = lang === "vi" ? "Công cụ trình duyệt miễn phí của DJAI Academy" : lang === "en" ? "DJAI Academy free browser tools" : "เครื่องมือฟรีบน browser จาก DJAI Academy";
...
function localeLinks(slug, lang, hub = false) {
  const paths = { th: hub ? `${basePath}/` : href(slug, "th"), en: hub ? `${basePath}/en/` : href(slug, "en"), vi: hub ? `${basePath}/vi/` : href(slug, "vi") };
  const labels = { th: "ไทย", en: "EN", vi: "VI" };
  return Object.entries(paths).filter(([locale]) => locale !== lang).map(([locale, path]) => `<a href="${path}" hreflang="${locale}">${labels[locale]}</a>`).join("");
}

function translateVietnameseHtml(html) {
  const replacements = [
    ["All tools", "Tất cả công cụ"], ["Community", "Cộng đồng"], ["PRIVATE BROWSER PROCESSING", "XỬ LÝ RIÊNG TƯ TRONG TRÌNH DUYỆT"], ["FREE CONVERTER", "CÔNG CỤ MIỄN PHÍ"], ["Your file stays on this device. The conversion engine downloads once when you start.", "File vẫn ở trên thiết bị. Bộ máy chuyển đổi được tải khi bạn bắt đầu."], ["Maximum recommended file size: 500 MB", "Dung lượng khuyến nghị tối đa: 500 MB"], ["Large videos need sufficient free memory and may take several minutes", "Video lớn cần đủ bộ nhớ trống và có thể mất vài phút"], ["Codec support varies; an unsupported source will show an error", "Khả năng hỗ trợ codec khác nhau; hệ thống sẽ báo lỗi nếu nguồn không được hỗ trợ"], ["Choose a media file", "Chọn file media"], ["Accepted input:", "Đầu vào hỗ trợ:"], ["Output quality", "Chất lượng đầu ra"], ["High quality", "Chất lượng cao"], ["Balanced", "Cân bằng"], ["Smaller file", "File nhỏ hơn"], ["Select a file to begin.", "Chọn file để bắt đầu."], ["Download converted file", "Tải file đã chuyển đổi"], ["How it works", "Cách sử dụng"], ["Privacy and limitations", "Quyền riêng tư và giới hạn"], ["MEDIA WORKFLOWS", "QUY TRÌNH MEDIA"], ["VIDEO WORKFLOWS", "QUY TRÌNH VIDEO"], ["Continue with another audio or video task", "Tiếp tục với công việc âm thanh hoặc video khác"], ["Continue with another video task", "Tiếp tục với công việc video khác"], ["Audio tools", "Công cụ âm thanh"], ["Common questions", "Câu hỏi thường gặp"], ["BUILD WITH DJAI", "XÂY CÙNG DJAI"], ["Want to build a tool like this one?", "Bạn muốn tự xây một công cụ như thế này?"], ["Join the Academy", "Tham gia Academy"], ["Free tools", "Công cụ miễn phí"], ["Audio and video tools", "Công cụ âm thanh và video"]
  ];
...
function render(slug, config, lang, pagePath, pageCopy) {
  ...
  const vietnameseAlternate = pagePath ? `${basePath}/vi/` : href(slug, "vi");
  const isEn = lang !== "th";
  const taskLinks = [
...
for (const config of videoTools) {
  const name = vietnameseMediaNames[config.slug];
  config.vi = {
    title: `${name} miễn phí online | DJAI Media Tools`,
    meta: `${name} miễn phí ngay trong trình duyệt. Video không bị upload, không cần đăng ký và kết quả không có watermark.`,
    h1: `${name} miễn phí`, ui: name,
    hero: `Xử lý ${name.toLowerCase()} trên thiết bị bằng FFmpeg WebAssembly và tải kết quả khi hoàn tất.`,
    why: `Khi nào nên dùng ${name.toLowerCase()}?`,
    note: "Dùng công cụ khi bạn cần xử lý nhanh một tệp media mà không muốn gửi nội dung lên máy chủ chuyển đổi.",
    faq: [
      { q: "File có được upload không?", a: "Không. Công cụ xử lý file cục bộ trong trình duyệt trên thiết bị của bạn." },
      { q: "Công cụ có miễn phí không?", a: "Có. Không cần đăng ký và tệp kết quả không có watermark." },
      { q: "Tại sao một số file không xử lý được?", a: "Khả năng hỗ trợ phụ thuộc vào codec, trình duyệt, bộ nhớ thiết bị, thời lượng và độ phân giải của file nguồn." },
      { q: "Xử lý video có làm giảm chất lượng không?", a: "Các tác vụ mã hóa lại có thể thay đổi chất lượng. Hãy kiểm tra file kết quả trước khi sử dụng." }
    ]
  };
}
...
function renderVideo(config, lang) {
  ...
  const vietnameseAlternate = href(config.slug, "vi");
  ...
  const schemas = [
    ...
    { "@context": "https://schema.org", "@type": "BreadcrumbList", itemListElement: [
      { "@type": "ListItem", position: 1, name: isEn ? "Free tools" : "เครื่องมือฟรี", item: `${origin}${lang === "vi" ? "/tools/vi/" : isEn ? "/tools/en/" : "/tools/"}` },
      { "@type": "ListItem", position: 2, name: isEn ? "Audio and video tools" : "เครื่องมือเสียงและวิดีโอ", item: `${origin}${basePath}/${lang === "th" ? "" : `${lang}/`}` },
...
for (const [slug, config] of Object.entries(tools)) for (const lang of ["th", "en", "vi"]) {
  ...
  const html = enhanceHead(render(slug, config, lang), {
    title: `${title} | DJAI Media Tools`, description, canonical, lang
  });
  writeFileSync(join(directory, "index.html"), lang === "vi" ? translateVietnameseHtml(html) : html);
}
for (const config of videoTools) for (const lang of ["th", "en", "vi"]) {
  const directory = join(publicDir, config.slug, ...(lang === "th" ? [] : [lang]));
  mkdirSync(directory, { recursive: true });
  const html = enhanceHead(renderVideo(config, lang), {
...
const vietnameseHub = ["Công cụ âm thanh và video miễn phí online", "Công cụ âm thanh và video miễn phí", "Chuyển đổi, cắt, nén, crop, resize, ghép, xoay hoặc trích xuất khung hình từ video và xử lý âm thanh trong trình duyệt mà không upload file."];
writeFileSync(join(publicDir, "vi", "index.html"), translateVietnameseHtml(enhanceHead(
  render(first[0], first[1], "vi", `${basePath}/vi/`, vietnameseHub),
  { title: `${vietnameseHub[0]} | DJAI Media Tools`, description: vietnameseHub[2], canonical: `${origin}${basePath}/vi/`, lang: "vi" }
)));
const uniqueToolCount = new Set([...Object.keys(tools), ...videoTools.map((item) => item.slug)]).size;
console.log(`Built ${uniqueToolCount * 3 + 3} media-tool pages.`);
